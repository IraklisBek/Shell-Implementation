...
int main()
{
    ...
    while(1){
        if(boolVar==0){
            printf("MyShell: %s$ ",directory); //  PRINT MySHell: "Directory"$
        ...
        //  ----------   COMMAND CD    ----------
                ...
                if (arg[1]==NULL || strcmp(arg[1],"~")==0){ // "cd" or "cd ~"
                    chdir("/home");  //home directory
                }else{
                    strcat(directory,"/");
                    strcat(directory,arg[1]); // directory + / + path
                    chdir(directory); // if fail ,directory->cwd
                }
                getcwd(directory,sizeof(directory)); // directory=cwd
            }
            //  ----------  FORK PROCESS   ----------
            else{
                int background;
